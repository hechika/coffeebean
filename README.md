...
- 히스토리 manifest는 `manifests/ios/hist/manifest_dev_2.0.68_build_2.plist`처럼 빌드별로 보존됩니다.
...
- 업로드된 IPA URL로 `manifests/ios/manifest_{env}.plist`의 IPA URL과 `bundle-version` 수정
- `manifests/ios/hist/manifest_{env}_{version}.plist` 생성 또는 갱신
...
- `manifests/ios/manifest_{env}.plist`의 IPA URL과 `bundle-version` 수정
- `manifests/ios/hist/manifest_{env}_{version}.plist` 생성 또는 갱신
- `releases.json`의 `ios.{env}` 배열에 버전, 배포일, 업데이트 노트, 이전 버전 다운로드 URL 추가 또는 갱신
...
3. 최신 manifest를 수정합니다.
   - DEV: `manifests/ios/manifest_dev.plist`
   - STG: `manifests/ios/manifest_stg.plist`
...
4. 같은 내용을 버전 히스토리 manifest로 복사합니다.
   - DEV 예시: `manifests/ios/hist/manifest_dev_2.0.67.plist`
   - STG 예시: `manifests/ios/hist/manifest_stg_2.0.67.plist`

5. `download.html`의 최신 iOS 설치 버튼은 항상 최신 manifest를 바라보게 유지합니다.

```text
itms-services://?action=download-manifest&url=https://hechika.github.io/coffeebean/manifests/ios/manifest_dev.plist
```
...
```json
{
  ...
  "downloadUrl": "itms-services://?action=download-manifest&url=https://hechika.github.io/coffeebean/manifests/ios/hist/manifest_dev_2.0.67.plist",
  "notes": ["퀵계좌이체 결제수단 추가"]
}
```
...
- `download.html`의 카드 버전은 `manifests/ios/manifest_dev.plist`, `manifests/ios/manifest_stg.plist`의 `bundle-version`에서 동적으로 표시됩니다.
- 최신 설치 버튼은 `manifests/ios/manifest_{env}.plist`를 사용합니다.
- 이전 버전 다운로드는 `manifests/ios/hist/manifest_{env}_{version}.plist`를 사용합니다.
...
- [ ] `manifests/ios/manifest_{env}.plist`의 IPA URL과 `bundle-version`을 최신 값으로 수정
- [ ] `manifests/ios/hist/manifest_{env}_{version}.plist`를 추가
