...
} from "./update-android-release.mjs";
...
export function printUsage() {
  console.log(`Usage:
  node scripts/deploy-android-release.mjs --env dev --version 2.0.68 --apk ./app-dev-debug.apk --released-at 2026-09-01 --note "업데이트 내용"

Options:
  --env <dev|stg|real>    Android 배포 환경
  --version <x.y.z>        앱 버전
  --apk <path>             업로드할 APK 파일 경로
  ...
  --no-commit              release JSON 변경사항을 커밋하지 않음
...
export function validateDeployArgs(args) {
  ...
  if (!args.apk) {
    throw new UsageError("--apk is required");
  }

  const apkPath = path.resolve(ROOT_DIR, args.apk);
  if (!fs.existsSync(apkPath)) {
    throw new UsageError(`APK file not found: ${apkPath}`);
  }

  if (!apkPath.endsWith(".apk")) {
    throw new UsageError("--apk must point to an .apk file");
  }
...
export function createRelease({ args, tag, apkPath, buildNumber }) {
  const title = `Android ${args.env.toUpperCase()} ${args.version} build ${buildNumber}`;
...
export function resolveBuildNumber(args) {
  ...
  const matchingBuilds = (releases.android?.[args.env] || [])
...
export function assetDownloadUrl({ repo, tag, apkPath }) {
  return `https://github.com/${repo}/releases/download/${encodeURIComponent(
    tag,
  )}/${encodeURIComponent(path.basename(apkPath))}`;
}
...
export function commitAndPush({ args, paths }) {
  ...
  run("git", [
    "commit",
    "-m",
    `Deploy Android ${args.env.toUpperCase()} ${args.version} build ${args["build-number"]}`,
  ]);
...
export function deployAndroidRelease(args, options = {}) {
  const apkPath = path.resolve(ROOT_DIR, args.apk);
  const buildNumber = resolveBuildNumber(args);
  args["build-number"] = String(buildNumber);
  const tag = `android-${args.env}-${args.version}-build.${buildNumber}`;
  const apkUrl = assetDownloadUrl({
    repo: args.repo,
    tag,
    apkPath,
  });
  const changedPaths = ["releases.json"];
  ...
  logStep(`GitHub Release에 APK 업로드 중 (build ${buildNumber})`);
  createRelease({ args, tag, apkPath, buildNumber });
  ...
  console.log("");
  console.log(`완료: Android ${args.env.toUpperCase()} ${args.version} build ${buildNumber}`);
